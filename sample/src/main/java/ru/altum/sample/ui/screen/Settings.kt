...
import ru.altum.sample.ui.theme.Color3
import javax.inject.Inject

@Serializable
internal class Settings(val value: String) : AnimatedDestination {
    @Composable
...
internal class SettingsViewModel @Inject constructor(
    ...
    private val args = savedStateHandle.requiredArgs<Settings>()
    ...
    fun onStreetClick() {
        appRouter.navigateTo(Street(_textState.value))
    }
...
internal fun SettingsScreen(
    ...
    Column(
        ...
        TopAppBar(
            title = { Text(text = "SettingsScreen") },
        ...
        Button(
            modifier = Modifier
                .fillMaxWidth()
                .padding(all = 16.dp),
            content = { Text("Back to Root") },
            onClick = { viewModel.onBackToRoot() }
        )
        Button(
            modifier = Modifier
                .fillMaxWidth()
                .padding(all = 16.dp),
            content = { Text("Street") },
            onClick = { viewModel.onStreetClick() }
        )
    }
}