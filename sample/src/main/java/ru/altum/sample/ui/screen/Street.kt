...
@Serializable
internal class Street(val value: String) : AnimatedDestination {
    @Composable
...
@HiltViewModel
internal class StreetViewModel @Inject constructor(
    savedStateHandle: SavedStateHandle,
    private val appRouter: AppRouter,
) : ViewModel() {

    private val args = savedStateHandle.requiredArgs<Street>()
    ...
    fun onMusicClick() {
        appRouter.navigateTo(Music(_textState.value))
    }

    fun onMailClick() {
        appRouter.navigateTo(Mail(_textState.value))
    }
    ...
    Column(
        ...
        Button(
            modifier = Modifier
                ...
                .padding(all = 16.dp),
            content = { Text("Music") },
            onClick = { viewModel.onMusicClick() }
        )
        Button(
            modifier = Modifier
                .fillMaxWidth()
